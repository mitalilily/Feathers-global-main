...
type ZoneStats = BaseStats & {
  label: string
  rawZoneName: string
  rawZoneCode: string
  courierStats: Record<string, BaseStats>
}

type PincodeStats = BaseStats & {
  pincode: string
  city: string
  state: string
  courierStats: Record<string, BaseStats>
...
const getCourierLabel = (value: string | null) => {
...
const getWeightValue = (row: OrderRow) =>
  toNumber(row.actualWeight ?? row.chargedWeight ?? row.weight)

const getDispatchDays = (row: OrderRow) => {
...
const getDeliveryDays = (row: OrderRow) => {
  const created = row.createdAt ?? parseDate(row.orderDate)
  const updated = row.updatedAt
  if (!created || !updated) return null
...
const detectColor = (productName: string, sku?: string | null) => {
...
const detectSize = (productName: string, sku?: string | null) => {
  const text = `${productName} ${sku ?? ''}`.toLowerCase()
...
const applyOrderStats = (stats: BaseStats, row: OrderRow) => {
  const status = lower(row.orderStatus)
...
const finalizeBaseStats = (stats: BaseStats) => {
  ...
  return {
    ...
    codDeliveryRate: stats.codOrders
      ? Number(((stats.codDelivered / stats.codOrders) * 100).toFixed(1))
      : 0,
...
const finalizeDimensionStats = (label: string, stats: BaseStats) => ({
  label,
  ...finalizeBaseStats(stats),
})

const buildPairs = (items: Record<string, BaseStats>) =>
  Object.entries(items)
    .map(([label, stats]) => finalizeDimensionStats(label, stats))
    .sort((a, b) => b.orders - a.orders || b.deliveryRate - a.deliveryRate)

const buildPairMap = (map: Map<string, BaseStats>) =>
  Array.from(map.entries())
    .map(([label, stats]) => finalizeDimensionStats(label, stats))
    .sort((a, b) => b.orders - a.orders || b.deliveryRate - a.deliveryRate)

const emptyZoneStats = (label: string, rawZoneName: string, rawZoneCode: string): ZoneStats => ({
  ...createBaseStats(),
  label,
  rawZoneName,
  rawZoneCode,
...
const emptyPincodeStats = (pincode: string, city: string, state: string): PincodeStats => ({
  ...createBaseStats(),
  pincode,
  city,
  state,
...
const resolveZoneIndex = async (orders: OrderRow[]) => {
  const uniquePincodes = Array.from(
    new Set(orders.map((row) => normalizeText(row.pincode).trim()).filter(Boolean)),
  )
  ...
  const pincodeMap = new Map<string, { label: string; zoneCode: string; zoneName: string }>()
  const stateMap = new Map<string, { label: string; zoneCode: string; zoneName: string }>()
  ...
  for (const row of zoneRows) {
    ...
    for (const state of toSafeArray(row.states)) {
      ...
      if (stateKey) {
        stateMap.set(stateKey, { label, zoneCode, zoneName })
      }
    }
  }
...
const pickZone = (
  row: OrderRow,
  zoneIndex: Awaited<ReturnType<typeof resolveZoneIndex>>,
) => {
  const pincodeKey = normalizeText(row.pincode).toLowerCase()
  const stateKey = normalizeText(row.state).toLowerCase()

  return (
    zoneIndex.pincodeMap.get(pincodeKey) ||
    zoneIndex.stateMap.get(stateKey) || {
      label: 'Unknown',
      zoneCode: 'Unknown',
      zoneName: 'Unknown',
    }
...
const topByOrders = <T extends { orders: number; deliveryRate?: number; rtoRate?: number }>(
  values: T[],
  limit = 5,
) =>
  [...values]
...
const topByRisk = <T extends { orders: number; rtoRate?: number }>(values: T[], limit = 5) =>
  [...values]
    .sort((a, b) => (b.rtoRate || 0) - (a.rtoRate || 0) || b.orders - a.orders)
    .slice(0, limit)

export const getAdminOpsAnalytics = async (filters: AnalyticsFilters = {}) => {
  const conditions = [sql`true`]

  if (filters.userId) {
    conditions.push(eq(b2c_orders.user_id, filters.userId))
  }

  if (filters.fromDate) {
    conditions.push(gte(b2c_orders.created_at, toStartOfDay(filters.fromDate)))
  }

  if (filters.toDate) {
    conditions.push(lte(b2c_orders.created_at, toEndOfDay(filters.toDate)))
  }
  ...
  if (filters.courier) {
    ...
    conditions.push(
      or(
        ...
        ilike(b2c_orders.integration_type, courierPattern),
      ) as any,
    )
  }
  ...
  const orders = (await db
    ...
    .from(b2c_orders)
    .where(and(...conditions))) as OrderRow[]

  const zoneIndex = await resolveZoneIndex(orders)
  ...
  const courierStats = new Map<string, BaseStats>()
  const zoneStats = new Map<string, ZoneStats>()
  const pincodeStats = new Map<string, PincodeStats>()
  const courierWeightStats = new Map<string, Record<string, BaseStats>>()
  const pincodeCourierStats = new Map<string, Record<string, BaseStats>>()
  const zoneCourierStats = new Map<string, Record<string, BaseStats>>()
  const weightStats = new Map<string, BaseStats>()
  const colorStats = new Map<string, BaseStats>()
  const priceStats = new Map<string, BaseStats>()
  ...
  for (const row of normalizedOrders) {
    ...
    const courierStatsBucket = getOrCreate(courierStats, courierKey, createBaseStats)
    applyOrderStats(courierStatsBucket, row)

    const zoneBucket = getOrCreate(zoneStats, zoneKey, () => emptyZoneStats(zoneKey, row.zone.zoneName, row.zone.zoneCode))
    applyOrderStats(zoneBucket, row)

    const pincodeBucket = getOrCreate(
      pincodeStats,
      pincodeKey,
    ...
    const zoneCourierBucket = getOrCreate(zoneCourierStats, zoneKey, () => ({} as Record<string, BaseStats>))
    ...
    const pincodeCourierBucket = getOrCreate(pincodeCourierStats, pincodeKey, () => ({} as Record<string, BaseStats>))
    ...
    const weightBucket = getOrCreate(weightStats, weightKey, createBaseStats)
    applyOrderStats(weightBucket, row)

    const courierWeightBucket = getOrCreate(courierWeightStats, courierKey, () => ({} as Record<string, BaseStats>))
    const courierWeightStat = getOrCreateRecord(courierWeightBucket, weightKey, createBaseStats)
    applyOrderStats(courierWeightStat, row)

    const priceBucket = getOrCreate(priceStats, priceKey, createBaseStats)
    applyOrderStats(priceBucket, row)

    const dispatchBucket = dispatchKey ? getOrCreate(dispatchStats, dispatchKey, createBaseStats) : null
    if (dispatchBucket) {
      applyOrderStats(dispatchBucket, row)
    }
    ...
    for (const category of tokens.categories) {
      applyOrderStats(getOrCreate(categoryStats, category, createBaseStats), row)
    }
    for (const sku of tokens.skus) {
      applyOrderStats(getOrCreate(skuStats, sku, createBaseStats), row)
    }
    for (const size of tokens.sizes) {
      applyOrderStats(getOrCreate(sizeStats, size, createBaseStats), row)
    }
    for (const color of tokens.colors) {
      applyOrderStats(getOrCreate(colorStats, color, createBaseStats), row)
    }
    if (tokens.colors.length === 0) {
      applyOrderStats(getOrCreate(colorStats, detectColorFromRow(row), createBaseStats), row)
    }
  }
  ...
  const ndrReasonStats = new Map<string, BaseStats>()
  for (const row of ndrReasonRows) {
    ...
    if (!order) continue
    const bucket = getOrCreate(ndrReasonStats, reason, createBaseStats)
    applyOrderStats(bucket, order)
  }
  ...
  const zoneRtoAnalytics = zoneOverview.map((zone) => {
    ...
    return {
      zone: zone.label,
      codRto: finalized.codRtoRate,
      prepaidRto: finalized.prepaidRtoRate,
    }
  })
  ...
  const zoneSpeed = zoneOverview.map((zone) => {
    const couriers = Object.entries(zoneCourierStats.get(zone.label) || {})
      ...
      .sort((a, b) => a.avgDeliveryDays - b.avgDeliveryDays || b.deliveryRate - a.deliveryRate || b.orders - a.orders)

    const bestCourier = couriers[0]
    return {
      zone: zone.label,
      bestCourier: bestCourier?.courier || 'Unknown',
      avgDays: bestCourier?.avgDeliveryDays || 0,
    }
  })
  ...
  const highRiskPincodes = topByRisk(
    Array.from(pincodeStats.values()).map((stats) => ({
  ...
  const pincodeCourierComparison = Array.from(pincodeCourierStats.entries())
    .flatMap(([pincode, courierMap]) =>
      Object.entries(courierMap).map(([courier, stats]) => ({
        pincode,
        courier,
        ...finalizeBaseStats(stats),
      })),
  ...
  const codFriendlyPincodes = Array.from(pincodeStats.values())
    .map((stats) => {
      ...
      return {
        pincode: stats.pincode,
        codDelivery: finalized.codDeliveryRate,
        codRto: finalized.codRtoRate,
      }
    })
    .filter((item) => item.codDelivery > 0)
    .sort((a, b) => b.codDelivery - a.codDelivery || a.codRto - b.codRto)
    .slice(0, 6)
  ...
  const courierRtoByWeight = matrixCouriers.map((courier) => {
    ...
    return {
      courier,
      ...Object.fromEntries(
        Object.entries(buckets).map(([slab, stats]) => [slab, finalizeBaseStats(stats).rtoRate]),
      ),
    }
  })
  ...
  const worstZone = [...zoneOverview].sort((a, b) => a.deliveryRate - b.deliveryRate || b.rtoRate - a.rtoRate)[0] || null
  ...
  return {
    ...
    data: {
      summary: {
        ...
        deliveryRate: finalizeBaseStats(overallStats).deliveryRate,
        rtoRate: finalizeBaseStats(overallStats).rtoRate,
        avgDeliveryDays: finalizeBaseStats(overallStats).avgDeliveryDays,
        avgDispatchDays: finalizeBaseStats(overallStats).avgDispatchDays,
        bestZone,
        worstZone,
        bestCourier,
      },
      ...
      dispatchDelay,
      guidance: [
        'West India -> Delhivery Best',
        'South India -> Ecom Best',
      ],
      filtersApplied: {
...
const detectColorFromRow = (row: OrderRow) => {
  const products = normalizeProductList(row.products)
  const first = products.find((item) => item && typeof item === 'object') as any
...
const buildZoneOverview = (zoneStats: Map<string, ZoneStats>) =>
  Array.from(zoneStats.entries())
    .map(([label, stats]) => ({
      ...
      bestCourier: Object.entries(stats.courierStats)
        .map(([courier, courierStats]) => ({
          courier,
          ...finalizeBaseStats(courierStats),
        }))
        .sort((a, b) => a.avgDeliveryDays - b.avgDeliveryDays || b.deliveryRate - a.deliveryRate || b.orders - a.orders)[0]?.courier || 'Unknown',
    }))
